...
const getNavItemsByRole = (role: 'admin' | 'superviseur' | 'citoyen' | 'agent') => {
  const commonAdminItems: NavItem[] = [
    ...
    },
    {
      ...
      name: "Gestion signalements",
      path: "/gestion-signalements",
    },
    {
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const citoyenItems: NavItem[] = [
    ...
    },
    {
      ...
      name: "Personnes disparues",
      path: "/citoyen/personnes-disparues",
    },
    {
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  if (role === 'admin') {
    return [...commonAdminItems, ...adminOnlyItems];
...
// Par défaut, on utilise le rôle de l'utilisateur connecté
...
const AppSidebar: React.FC = () => {
  ...
  // Obtenir les items du menu selon le rôle
  const navItems: NavItem[] = user ? getNavItemsByRole(user.role as 'admin' | 'superviseur' | 'citoyen') : [];
  ...
  return (
    <>
          ...
          <Link to="/dashboard">
            {isExpanded || isHovered || isMobileOpen ? (
              <div className="flex items-center gap-2">
                ...
                />
                <div>
                  <span className="font-bold text-gray-800 dark:text-white/95 text-lg leading-tight block">
                    TOGO-SecureNet
                  </span>
      ...
      {/* Modale de confirmation de déconnexion - rendue à l'extérieur du sidebar */}
      {showLogoutModal && (
